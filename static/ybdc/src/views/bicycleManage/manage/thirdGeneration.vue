  ...
  <div>
      <div class='search-bar cl'>
          ...
          <div class="search-bar-grid fl">
            ...
            <el-select class='form-input'  style="width:260px;"  size="small"  v-model="selectedRule" multiple  @change='selectRule'  placeholder="请选择">
               ...
               <el-option  label="无异常" value="none">无异常</el-option>
               <el-option  label="骑行超时" value="rideTimeout">骑行超时</el-option>
                <el-option  label="报文异常" value="dataException">报文异常</el-option>
               <el-option  label="低电量" value="lowBattery">低电量</el-option>
               <el-option  label="低频" value="lowFrequency">低频</el-option>
            </el-select>
      ...
      <el-row class='table-content'>
        ...
        >
          ...
          <el-table-column
            ...
            <template scope="scope">
              <span style='color:red;'
                v-if = 'scope.row.battery<rulesTransform.lowBattery'
          ...
          <el-table-column
            ...
            prop="scope.row.endPlace"
            label="最新位置"
            show-overflow-tooltip>
            <template scope="scope">
              <el-button
                v-if = 'scope.row.endPlace'
                size="small"
                @click="showLocationMap(scope.row.endLat,scope.row.endLon)"
                >关锁位置</el-button>
                <el-button
                  v-if = 'scope.row.startPlace&&!scope.row.endPlace'
                  size="small"
                  @click="showLocationMap(scope.row.startLat,scope.row.startLon)"
                  >开锁位置</el-button>
    ...
    <el-dialog title="异常规则设置" v-model="showAnomaly" >
      <el-form >
        ...
        <el-form-item label="报文异常：">
            距离上一次报文时长大于&nbsp;<el-input v-model="rulesTransform.dataException" type="number" size='small' class='w-40' auto-complete="off"></el-input>分钟
        </el-form-item>
      ...
      <div class="dialog-content">
        ...
        <el-form-item label="MAC地址：" :label-width="formLabelWidth">
          ...
          <el-form-item label="设备序列号：" :label-width="formLabelWidth">
            <el-input :disabled="disabled" size='small' style='width:auto' v-model="bicycleDetailInfo.deviceId" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="IMEI：" :label-width="formLabelWidth">
            <el-input :disabled="disabled" size='small' style='width:auto' v-model="bicycleDetailInfo.imei" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="运营商：" :label-width="formLabelWidth">
            <el-input :disabled="disabled" size='small' style='width:auto' v-model="bicycleDetailInfo.operator" auto-complete="off"></el-input>
          </el-form-item>
          <!-- <el-form-item label="剩余咨询费用：" :label-width="formLabelWidth" >
   ...
   <el-dialog title="批量导入"  v-model="showUpload">
                      ...
                      action="/bicycle/manage/fileUpload/3"
                      :file-list="fileList3">
     ...
     <div class="dialog-content">
       <el-form  label-position='left'>
         ...
         <el-form-item label="车辆城市：" :label-width="formLabelWidth">
           <el-select class='form-input' style="width:161px;"  v-model="addBicycleParam.cityCode" size='small' placeholder="请选择">
              <el-option
                v-for="item in cityList"
                :label="item.name"
                :value="item.code">
         ...
         <el-form-item label="MAC地址：" :label-width="formLabelWidth" >
           <el-input  size='small' style='width:auto' v-model="addBicycleParam.mac" auto-complete="off" @blur="checkMacUnique(ddBicycleParam.mac)"></el-input>
         </el-form-item>
       </el-form>
     </div>
    ...
    methods:{
      ...
      queryBicycleList(){
        ...
        getBicycleByPagenation(batch,this.bicycleListParam,data => {
          ...
          //  this.bicycleDetail[0].rideTime = 100
      ...
      excelExport(){
        ...
        window.open('/bicycle/manage/excelExport/3?'+str)
