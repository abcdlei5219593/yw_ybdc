      ...
      <el-col :span='22' class="content">
        <el-row>
                ...
                <div class="detail-container">
                    ...
                    <div class="layout-mgt-30">
                        <el-row>
                            ...
                            <el-col :span='18'>
                                <el-select v-model="recoverReason" clearable placeholder="回收原因" size="small">
                                      <el-option v-for="item in recoverReasons" :label="item.name" :value="item.code"></el-option>
                                  </el-select>
                            </el-col>
                        </el-row>
                        <el-row>
                        ...
                        <el-row>
                            ...
                            <el-col :span='18'>
                                <el-select v-model="dealer" clearable placeholder="请选择" size="small" @change="dealerChange">
          ...
          <el-col :span='12' class="relative">
                ...
                <el-button style="position:absolute;top:30px;left:30px" @click ='showAllBicylce'>车辆</el-button>
                <el-button style="position:absolute;top:30px;margin-left:2%;left:90px" @click ='showAlamBicylce'>告警</el-button>
          </el-col>
        </el-row>
      </el-col>
</template>
...
<script>
  ...
  import {baseUrl,queryCodeByTaskId,getAlamBicylce,findOperationMaintenancerByCity,queryTaskGroupList,queryTaskTypes,queryMalfunctionTypes,queryTaskSources,queryRecoverReasons,findAllUser,createTaskGroup,getTeams,getAllEntityName,getBicylceStatus,queryTaskFiles,getAllPolygon,getAllcoordinate,getAllBicycles} from '../../service/getData'
...
export default {
  ...
  data () {
    return {
      ...
      alamMarkerIsShow:false, //告警车辆是否显示 默认不显示
      alamMarker:[]
    }
  },
  mounted () {
      ...
      getAlamBicylce('',data => {
        ...
        for(let i=0;i<data.result.result.length;i++){
          var point = new BMap.Point(data.result.result[i].lon,data.result.result[i].lat);
          var marker = new BMap.Marker(point,{icon:new BMap.Icon("http://trojian-cdn-play.oss-cn-hangzhou.aliyuncs.com/ioc/运维图标06.png", new BMap.Size(25,25))});  // 创建标注
          var label = new BMap.Label(data.result.result[i].code,{offset:new BMap.Size(20,-40)});
          label.setStyle(
             {color : "white",
                 padding:"5px",
                 fontSize : "16px",
  ...
  methods:{
            ...
            queryTaskGroupList(dataParam,function(data){
              if(data.success==true){
                ...
                for(let i=0;i<subMission.tableData.length;i++){
                  ...
                  var urlImg;
                  if(subMission.tableData[i].taskTypeCode==200){
                    urlImg= subMission.tableData[i].isRead==0?"http://trojian-cdn-play.oss-cn-hangzhou.aliyuncs.com/png/19.png":"http://trojian-cdn-play.oss-cn-hangzhou.aliyuncs.com/png/17.png";
                  }
        ...
        create(){
          ...
          queryRecoverReasons('',function(data){
            console.log("queryRecoverReasons----------"+data);
            subMission.recoverReasons = data.result.list;
          });
        },
        exportNum(){
        ...
        showAlamBicylce () { //显示告警车辆

          if(this.alamMarkerIsShow){
            this.alamMarkerIsShow =false;
            markerClusterer.clearMarkers();
          }else{
            this.alamMarkerIsShow =true;
            markerClusterer = new BMapLib.MarkerClusterer(map, {markers: this.alamMarker});
          }
        }
  }
}
